 ...
 * @Description:
 * @Author: Alvin
 * @Date: 2023-05-27 14:17:00
 * @LastEditTime: 2023-06-06 15:26:10
 */
...
#define CONSOLE_NR 8 // CONSOLE_NR常量表示控制台数量
...
/**
 * @: 将光标移动 n 个位置
 * @param {console_t *} console
 * @param {int} n
 * @return {*}
 */
static void move_forword(console_t *console, int n)
{
    for (int i = 0; i < n; i++)
    {
...
/**
 * @: 在指定的控制台对象上显示一个字符
 * @param {console_t *} console 要显示字符的控制台对象
 * @param {char} c 要显示的字符
 * @return {*}
 */
...
int console_init(int idx)
{
    ...
    if (idx == 0) // 如果是第 0 块光标
    {
        int consor_pos = read_cursor_pos();
        console->cursor_row = consor_pos / console->display_cols;
        console->cursor_col = consor_pos % console->display_cols;
    }
    else
    {
