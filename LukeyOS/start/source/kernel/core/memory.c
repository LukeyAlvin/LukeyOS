...
/**
 * @description: 查找与给定虚拟地址对应的页表条目（PTE）
 * @return {*}
...
/**
 * @description: 用于在页表中创建一个虚拟地址到物理地址的映射
 * @return {*}
 * @param {pde_t *} page_dir    要创建映射的页目录表
 * @param {uint32_t} vaddr      要映射的虚拟地址的起始地址
...
{
    // 起始地址的获取在链接脚本中：source/kernel/kernel.lds
    extern uint8_t s_text[], e_text[], s_data[], e_data[]; // 内核代码段、数据段的起始和终点地址
    extern uint8_t kernel_base[];                          // 内核的起始地址  0x0
